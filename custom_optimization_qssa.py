...
from custom_optimization_qssa_settings import *
#%%
plt.rcParams.update({
    "text.usetex": True,
    "font.family": "serif",
...
#%%
#define the observable NF_obs
def NF(p):
	model.setParameters(p)
	t = amici.runAmiciSimulation(model, solver, None).t
	R = amici.runAmiciSimulation(model, solver, None).x[:,0]
	(N_var,t_wait,gV,gP,fac,sigma,L,k_base,t0,a1,mu1,a2,mu2,a3,mu3,a4,mu4,
        a5,mu5,a6,mu6,a7,mu7,a8,mu8,a9,mu9,a10,mu10,a11,mu11,a12,mu12,a13,mu13,a14,mu14,a15,mu15,a16,mu16,a17,mu17,a18,mu18,a19,
	...
	a59,mu59,a60,mu60,kUmax,kUmin,steep,cliffstart,kR,nves,nsites) = 10**p
	return N_var*(10**(L*(1-exp(-k_base*((t*fac-t_wait)-t0))))+
...
if (objective == 'peak_values_NF'):
    def f_obj(param):
        ...
        return (((NF_data[NF_peaks_data] - 
                         NF_model[NF_peaks_model][:60])**2).sum()) 
...
if (objective == 'peak_values_NF_and_current'):
    def f_obj(param):
        param_Id = list(model.getParameterIds()[i] for i in param_indices)
        model.setParameterById(dict(zip(param_Id, param)))
        all_params = np.array(model.getParameters())
        ...
        return (((current_data[top_peaks_data] - current_model[top_peaks_model][:59])**2).sum()
                + ((current_data[bottom_peaks_data] - current_model[bottom_peaks_model][:60])**2).sum()
                + 1e-28 * ((NF_data[NF_peaks_data] - NF_model[NF_peaks_model][:60])**2).sum())

if (objective == 'frechet_NF'):
    def f_obj(param):
        param_Id = list(model.getParameterIds()[i] for i in param_indices)
        ...
        t = amici.runAmiciSimulation(model, solver).t
       	NF_model = NF(all_params)
        NF_peaks_model = find_peaks(NF_model)[0]
        return (frechet_objective(t,NF_model,NF_data,0,250)[0]+ 
                   frechet_objective(t,NF_model,NF_data,250,500)[0]+
                   frechet_objective(t,NF_model,NF_data,500,750)[0]+
                    frechet_objective(t,NF_model,NF_data,750,1000)[0]+
                    frechet_objective(t,NF_model,NF_data,1000,1250)[0]+
                    frechet_objective(t,NF_model,NF_data,1250,1500)[0]+
                    frechet_objective(t,NF_model,NF_data,1500,1750)[0]+
                    frechet_objective(t,NF_model,NF_data,1750,2000)[0]+
                    frechet_objective(t,NF_model,NF_data,2000,2250)[0]+
                    frechet_objective(t,NF_model,NF_data,2250,2500)[0]+
                    frechet_objective(t,NF_model,NF_data,2500,2750)[0]+
                    frechet_objective(t,NF_model,NF_data,2750,3000)[0])
...
if (objective == 'frechet_NF_and_current'):
    def f_obj(param):
        param_Id = list(model.getParameterIds()[i] for i in param_indices)
        ...
        return (frechet_objective(t,current_model,current_data,0,250)[0]+ 
                   ...
                   frechet_objective(t,current_model,current_data,500,750)[0]+
                    ...
                    1e-12*(frechet_objective(t,NF_model,NF_data,250,500)[0]+
                    frechet_objective(t,NF_model,NF_data,500,750)[0]+
                    frechet_objective(t,NF_model,NF_data,750,1000)[0]+
                    frechet_objective(t,NF_model,NF_data,1000,1250)[0]+
                    frechet_objective(t,NF_model,NF_data,1250,1500)[0]+
...
def optimize_params(param_0):
	opt_ = minimize(f_obj, param_0, method = opt_algotrithm, bounds=None, tol=1e-30).x
	return opt_

#%%
#randomly selects initial values for parameters 
params_0 = np.vstack([np.log10(np.random.uniform(0,0.3*nom_val,
...
for fit in ['NF','Current']:
    ...
    for (n1,n) in [(0,3000),(0,500),(500,1000),(1000,1500),(1500,2000),(2000,2500),(2500,3000)]:
        ...
        #all_params = model.getParameters()
        #plt.plot(times[:],NF(np.array(all_params))[:],'.',markersize = 0.5,linewidth = 1.1)
        plt.title('Model fit')
        plt.xlabel('Time ($t$)')
        plt.ylabel(f'{fit}')
        plt.legend()
...
np.save(f'{run_dir}/NF_best.npy',NF(np.array(all_params)))
np.save(f'{run_dir}/Current_best.npy',current(NF(np.array(all_params))))
